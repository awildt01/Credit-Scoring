...
**7.3 PD Modeling:**
- Bei der **PD-Modellierung** schloss ich zunächst Variablen aus, die zum Zeitpunkt der Vorhersage nicht verfügbar wären, um **Datenlecks (Data Leakage)** zu vermeiden. Beispiele hierfür sind die bewilligte Kreditsumme **(funded amount)** oder Gesamtzahlungen **(total payments)**. Zusätzlich entfernte ich Variablen, die während der Exploratorischen Datenanalyse (EDA) keine Trennschärfe (discriminatory power) aufwiesen.
...
- Nehmen wir an, ein Jahr ist vergangen, seit wir unser PD-Modell entwickelt haben. Obwohl unwahrscheinlich, könnten sich die aktuellen Kreditantragsteller stark von jenen unterscheiden, die zum Training des Modells dienten. Wir müssen prüfen, ob das PD-Modell noch valide ist.
- Weicht die Population der Neuantragsteller zu stark von der Trainingspopulation ab, drohen verheerende Folgen. In diesem Fall ist eine Neuentwicklung des Modells erforderlich.
- Ich führte ein Jahr nach Modellerstellung ein Modell-Monitoring des PD-Modells mit Kreditdaten aus 2015 durch. Ziel ist es, die zeitliche Stabilität der Antragsteller-Merkmale zu prüfen. Grundannahme von Kreditrisikomodellen: Zukünftige Daten ähneln vergangenen Daten. Bei signifikanten Populationsverschiebungen kann Retraining nötig werden. Um Unterschiede zwischen Ist (Trainingsdaten) und Soll (Monitoring-Daten) zu quantifizieren, wurde für jede Variable der Population Stability Index (PSI) berechnet.
- Der "Startstatus" (initial list status) wies den höchsten PSI auf (nahe 0.25), was auf eine deutliche Veränderung der Antragstellerpopulation hindeutet. Diese Änderung ist jedoch eher auf strategische Anpassungen der Bank zurückzuführen als auf geänderte Kundenmerkmale.
